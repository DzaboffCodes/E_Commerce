...
const createCart = async (req, res) => {
  try {
    ...
    errorResponse(
      res,
      "Server error during cart creation",
      500,
      "CREATE_CART_ERROR",
    );
  }
  ...
  try {
    ...
    if (cartCheck.rows.length === 0) {
      return errorResponse(
        res,
        "Cart not found or does not belong to you",
        404,
        "CART_NOT_FOUND",
      );
    }
    ...
    errorResponse(
      res,
      "Server error while fetching cart",
      500,
      "FETCH_CART_ERROR",
    );
  }
...
const addToCart = async (req, res) => {
  try {
    ...
    errorResponse(
      res,
      "Server error during adding product to cart",
      500,
      "ADD_TO_CART_ERROR",
    );
  }
...
const removeFromCart = async (req, res) => {
  try {
    ...
    if (currentQty > 1) {
      await db.query("UPDATE cart_items SET qty = qty - 1 WHERE id = $1", [
        itemId,
      ]);
    ...
    errorResponse(
      res,
      "Server error during cart item deletion",
      500,
...
const checkoutCart = async (req, res) => {
  try {
    ...
    if (cartItems.rows.length === 0) {
      return errorResponse(
        res,
        "Cart not found, empty, or does not belong to you",
        404,
        "CART_CHECKOUT_ERROR",
      );
    }
...
const createPaymentIntent = async (req, res) => {
  try {
    ...
    if (cartItems.rows.length === 0) {
      return errorResponse(res, "Cart not found or empty", 404, "CART_EMPTY");
    }
