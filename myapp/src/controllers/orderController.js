...
const updateOrderStatus = async (req, res) => {
  try {
    ...
    const orderCheck = await db.query("SELECT id FROM orders WHERE id = $1", [
      id,
    ]);
    ...
    errorResponse(
      res,
      "Server error during order status update",
      500,
      "UPDATE_ORDER_ERROR",
    );
  }
...
const createOrderPaymentIntent = async (req, res) => {
  try {
    ...
    const orderResult = await db.query(
      "SELECT id, total, status FROM orders WHERE id = $1 AND userid = $2",
      [id, userId],
    );
    ...
    errorResponse(
      res,
      "Server error creating order payment intent",
      500,
      "ORDER_PAYMENT_INTENT_ERROR",
    );
  }
};
...
const confirmOrderPayment = async (req, res) => {
  try {
    ...
    const orderResult = await db.query(
      "SELECT id, status FROM orders WHERE id = $1 AND userid = $2",
      [id, userId],
    );
    ...
    if (String(paymentIntent.metadata.orderId) !== String(id)) {
      return errorResponse(
        res,
    ...
    const updated = await db.query(
      "UPDATE orders SET status = $1, modified = CURRENT_TIMESTAMP WHERE id = $2 AND userid = $3 RETURNING *",
      ["successful", id, userId],
    );

    successResponse(res, { order: updated.rows[0] }, "Order payment confirmed");
  } catch (error) {
    console.error("Error confirming order payment:", error);
    errorResponse(
      res,
      "Server error confirming order payment",
      500,
      "CONFIRM_ORDER_PAYMENT_ERROR",
    );
  }
};
